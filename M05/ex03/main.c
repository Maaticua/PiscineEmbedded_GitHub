...
int main(void)
{
	...
	ADMUX |= (1 << REFS0) | (1 << REFS1) | (1 << MUX3); // on active le internal 1.1 Volatge Reference, obligatoire pour que la mesure de temp fonctionne p256 (le tension de reference) p257 et on active le onchip temperature sensor qui se trouve sur le ADC8
	...
	while(1)
	{
		...
		i = ADC;
		//	calcul de la pente et de l'offset de la relation lineaire entre voltage et degre a partir
		//	des 3 points de la courbe donner en exemple ADC = a * T + b
		//	a = (adc2 - adc1) / (T1 - T2) = 1.22, b = adc - a * T = 324.31 p256 + https://clg-cousteau-mery.ac-versailles.fr/IMG/pdf/3e-Chap5-Cours-Fonctions_lineaires_et_affines.pdf
		uart_putdecistr(((i - 324) * 100) / 122);
		uart_printstr("\r\n");
		_delay_ms(20);
	}
}
